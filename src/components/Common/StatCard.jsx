const StatCard = ({ title, value, icon: Icon, color = 'green', subtitle, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`relative overflow-hidden rounded-xl border p-4 sm:p-5 text-left w-full transition-all duration-300 hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] ${colorClasses[color] || colorClasses.green}`}
    >
