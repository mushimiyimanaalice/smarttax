...
const AdminMomoTransactions = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl sm:text-2xl font-bold text-slate-800">MoMo Transactions</h1>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="text-right p-3 font-medium text-slate-600">Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filtered.map((t, i) => (
                <tr key={t._id || i} className="hover:bg-slate-50">
                  ...
                  <td className="p-3 text-right font-medium text-slate-800">RWF {t.amount?.toLocaleString()}</td>
                  <td className="p-3 text-right">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[t.status] || 'bg-slate-100 text-slate-600'}`}>
                      {t.status}
                    </span>
                  </td>
